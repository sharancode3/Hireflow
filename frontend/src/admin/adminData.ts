...
const DUMMY_RECRUITERS: RecruiterItem[] = [
  {
    userId: "demo_rec_1",
  ...
  },
  {
    ...
    updatedAt: "2026-03-16T07:20:00.000Z",
  },
  {
    userId: "demo_rec_3",
    email: "recruiter.gamma@example.com",
...
function filterDummyRecruiters(status: "ALL" | RecruiterApprovalStatus, search: string): RecruiterItem[] {
  const needle = search.trim().toLowerCase();
  return DUMMY_RECRUITERS.filter((item) => {
    if (status !== "ALL" && item.status !== status) return false;
    if (!needle) return true;
    return [item.fullName, item.email, item.companyName, item.designation]
      .join(" ")
      .toLowerCase()
      .includes(needle);
  });
...
export async function fetchRecruiters(status: "ALL" | RecruiterApprovalStatus, search: string): Promise<RecruiterItem[]> {
  ...
  if (ids.length === 0) return filterDummyRecruiters(status, search);
  ...
  return mapped.length ? mapped : filterDummyRecruiters(status, search);
}
