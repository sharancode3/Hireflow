...
const STORAGE_KEY = "hireflow_recruiter_post_job_draft";
...
function loadDraftFromStorage(): DraftJob | null {
...
export function RecruiterPostJobPage() {
  const { token } = useAuth();
  const [persistedDraft] = useState<DraftJob | null>(() => loadDraftFromStorage());

  const [title, setTitle] = useState(persistedDraft?.title ?? defaultDraft.title);
  const [company, setCompany] = useState(persistedDraft?.company ?? defaultDraft.company);
  const [location, setLocation] = useState(persistedDraft?.location ?? defaultDraft.location);
  const [workplaceType, setWorkplaceType] = useState<DraftJob["workplaceType"]>(
    persistedDraft?.workplaceType ?? defaultDraft.workplaceType
  );
  const [industry, setIndustry] = useState(persistedDraft?.industry ?? defaultDraft.industry);
  const [experienceLevel, setExperienceLevel] = useState<DraftJob["experienceLevel"]>(
    persistedDraft?.experienceLevel ?? defaultDraft.experienceLevel
  );
  const [employmentType, setEmploymentType] = useState(persistedDraft?.employmentType ?? defaultDraft.employmentType);
  const [salaryMin, setSalaryMin] = useState(persistedDraft?.salaryMin ?? defaultDraft.salaryMin);
  const [salaryMax, setSalaryMax] = useState(persistedDraft?.salaryMax ?? defaultDraft.salaryMax);
  const [currency, setCurrency] = useState(persistedDraft?.currency ?? defaultDraft.currency);
  const [description, setDescription] = useState(persistedDraft?.description ?? defaultDraft.description);
  const [requirements, setRequirements] = useState(persistedDraft?.requirements ?? defaultDraft.requirements);
  const [deadline, setDeadline] = useState(persistedDraft?.deadline ?? defaultDraft.deadline);
  const [skills, setSkills] = useState(persistedDraft?.skills ?? defaultDraft.skills);
  const [screeningQuestions, setScreeningQuestions] = useState(persistedDraft?.screeningQuestions ?? defaultDraft.screeningQuestions);
